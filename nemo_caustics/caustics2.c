 ...
 * details may be found in the README. A stand-alone 
 *
...
local static const double caustics_G = 1.0;
...
const int X = 0;
const int caustics_Y = 1;
const int caustics_Z = 2;
...
void gfield_close(double rho, double z, int n, double *rfield, double *zfield) {
  ...
  // roots stored in re[]
  double factor = -8.0 * M_PI * caustics_G * rate_n[n] * 4498.6589 / (rho * V_n[n] * 1.0226831);
...
void gfield_far(double rho, double z, int n, double *rfield, double *zfield) {
  ...
  double A_n = (8.0 * M_PI * caustics_G * rate_n[n] * 4498.6589) / (V_n[n] * 1.0226831);  //convert from M_sun/yr*sr to m_sim/gyr*sr with 4498 and km/s to kpc/gyr with 1.0226
...
// call this from your own potential file
void apply_caustic_pot(double *pos, double *acc, double *pot) {
  ...
  rfield = 0.0;
  zfield = 0.0;
  rho = sqrt(pos[caustics_X]*pos[caustics_X] + pos[caustics_Y]*pos[caustics_Y]);
  z = pos[caustics_Z];
  ...
  acc[caustics_X] += rfield * pos[caustics_X] / rho;
  acc[caustics_Y] += rfield * pos[caustics_Y] / rho;
  acc[caustics_Z] += zfield;
